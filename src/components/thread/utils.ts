...
const TOOL_TRANSLATION_KEYS = new Map([
  ['execute-command', 'executeCommand'],
  ['check-command-output', 'checkCommandOutput'],
  ['terminate-command', 'terminateCommand'],
  ['list-commands', 'listCommands'],
  
  ['create-file', 'createFile'],
  ['delete-file', 'deleteFile'],
  ['full-file-rewrite', 'rewriteFile'],
  ['str-replace', 'editingText'],
  ['str_replace', 'editingText'],
  
  ['browser-click-element', 'clickingElement'],
  ['browser-close-tab', 'closingTab'],
  ['browser-drag-drop', 'draggingElement'],
  ['browser-get-dropdown-options', 'gettingOptions'],
  ['browser-go-back', 'goingBack'],
  ['browser-input-text', 'enteringText'],
  ['browser-navigate-to', 'navigatingToPage'],
  ['browser-scroll-down', 'scrollingDown'],
  ['browser-scroll-to-text', 'scrollingToText'],
  ['browser-scroll-up', 'scrollingUp'],
  ['browser-select-dropdown-option', 'selectingOption'],
  ['browser-click-coordinates', 'clickingCoordinates'],
  ['browser-send-keys', 'pressingKeys'],
  ['browser-switch-tab', 'switchingTab'],
  ['browser-wait', 'waiting'],

  ['execute-data-provider-call', 'callingDataProvider'],
  ['execute_data_provider_call', 'callingDataProvider'],
  ['get-data-provider-endpoints', 'gettingEndpoints'],
  
  ['deploy', 'deploying'],
  ['ask', 'ask'],
  ['complete', 'completingTask'],
  ['crawl-webpage', 'crawlingWebsite'],
  ['expose-port', 'exposingPort'],
  ['scrape-webpage', 'scrapingWebsite'],
  ['web-search', 'searchingWeb'],
  ['see-image', 'viewingImage'],
  
  ['call-mcp-tool', 'externalTool'],

  ['update-agent', 'updatingAgent'],
  ['get-current-agent-config', 'gettingAgentConfig'],
  ['search-mcp-servers', 'searchingMcpServers'],
  ['get-mcp-server-tools', 'gettingMcpServerTools'],
  ['configure-mcp-server', 'configuringMcpServer'],
  ['get-popular-mcp-servers', 'gettingPopularMcpServers'],
  ['test-mcp-server-connection', 'testingMcpServerConnection'],

  // V2 versions
  ['execute_command', 'executeCommand'],
  ['check_command_output', 'checkCommandOutput'],
  ['terminate_command', 'terminateCommand'],
  ['list_commands', 'listCommands'],
  
  ['create_file', 'createFile'],
  ['delete_file', 'deleteFile'],
  ['full_file_rewrite', 'rewriteFile'],
  ['str_replace', 'editingText'],
  
  ['browser_click_element', 'clickingElement'],
  ['browser_close_tab', 'closingTab'],
  ['browser_drag_drop', 'draggingElement'],
  ['browser_get_dropdown_options', 'gettingOptions'],
  ['browser_go_back', 'goingBack'],
  ['browser_input_text', 'enteringText'],
  ['browser_navigate_to', 'navigatingToPage'],
  ['browser_scroll_down', 'scrollingDown'],
  ['browser_scroll_to_text', 'scrollingToText'],
  ['browser_scroll_up', 'scrollingUp'],
  ['browser_select_dropdown_option', 'selectingOption'],
  ['browser_click_coordinates', 'clickingCoordinates'],
  ['browser_send_keys', 'pressingKeys'],
  ['browser_switch_tab', 'switchingTab'],
  ['browser_wait', 'waiting'],

  ['execute_data_provider_call', 'callingDataProvider'],
  ['get_data_provider_endpoints', 'gettingEndpoints'],
  
  ['deploy', 'deploying'],
  ['ask', 'ask'],
  ['complete', 'completingTask'],
  ['crawl_webpage', 'crawlingWebsite'],
  ['expose_port', 'exposingPort'],
  ['scrape_webpage', 'scrapingWebsite'],
  ['web_search', 'searchingWeb'],
  ['see_image', 'viewingImage'],
  
  ['call_mcp_tool', 'externalTool'],

  ['update_agent', 'updatingAgent'],
  ['get_current_agent_config', 'gettingAgentConfig'],
  ['search_mcp_servers', 'searchingMcpServers'],
  ['get_mcp_server_tools', 'gettingMcpServerTools'],
  ['configure_mcp_server', 'configuringMcpServer'],
  ['get_popular_mcp_servers', 'gettingPopularMcpServers'],
  ['test_mcp_server_connection', 'testingMcpServerConnection'],
]);
...
// New function that returns translation key for internationalization
export function getToolNameTranslationKey(toolName: string): string | null {
  if (toolName?.startsWith('mcp_')) {
    // For MCP tools, we'll handle them separately
    return null;
  }
  
  return TOOL_TRANSLATION_KEYS.get(toolName) || null;
}

// Function to get localized tool name using translation function
export function getLocalizedToolName(toolName: string, t: (key: string) => string): string {
  if (toolName?.startsWith('mcp_')) {
    const parts = toolName.split('_');
    if (parts.length >= 3) {
      const serverName = parts[1];
  ...
  const translationKey = TOOL_TRANSLATION_KEYS.get(toolName);
  if (translationKey) {
    return t(`toolViews.toolNames.${translationKey}`);
  }
