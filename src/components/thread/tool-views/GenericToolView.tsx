      ...
      <CardHeader className="h-6 bg-[#FFFFFF] dark:bg-[#202426] backdrop-blur-sm px-6 mb-0 gap-0">
        <div className="flex flex-row items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="relative flex items-center justify-center h-5 w-5 p-0 rounded-lg bg-gradient-to-br from-orange-500/20 to-orange-600/10 border border-orange-500/20">
              <Wrench className="w-3 h-3 text-orange-500 dark:text-orange-400" />
            </div>
            <div>
          ...
          {!isStreaming && (
            ...
            >
              ...
              {isSuccess ? t('toolViews.generic.toolExecutedSuccessfully') : t('toolViews.generic.toolExecutionFailed')}
            </Badge>
          )}
        </div>
      </CardHeader>
      ...
      <CardContent className="relative p-0 pt-4 pb-10 m-6 mt-2 h-full flex-1 overflow-hidden bg-[#F2F5FF] dark:bg-[#16191A] border-1 border-solid border-[#EDEDED] dark:border-[#2A2F31] rounded-2xl">
        {isStreaming ? (
          <LoadingState
            ...
            title={t('toolViews.common.executing')}
            filePath={name}
            showProgress={true}
          />
          ...
          <div className="flex flex-col items-center justify-center h-full py-12 px-6 bg-gradient-to-b from-white to-zinc-50 dark:from-zinc-950 dark:to-zinc-900">
            ...
            <h3 className="text-xl font-semibold mb-2 text-zinc-900 dark:text-zinc-100">
              {t('toolViews.common.noContentAvailable')}
            </h3>
            <p className="text-sm text-zinc-500 dark:text-zinc-400 text-center max-w-md">
              {t('toolViews.common.noToolContentDescription')}
            </p>
          </div>
        )}
