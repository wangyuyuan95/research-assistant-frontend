        ...
        <div className="flex flex-row items-center justify-between">
          ...
          {!isStreaming && (
            ...
            >
              ...
              {isSuccess ? t('toolViews.webCrawl.crawlingCompleted') : t('toolViews.webCrawl.crawlingFailed')}
            </Badge>
          )}
        </div>
      </CardHeader>
        ...
        {isStreaming ? (
          <div className="flex flex-col items-center justify-center h-full py-12 px-6 bg-gradient-to-b from-white to-zinc-50 dark:from-zinc-950 dark:to-zinc-900">
            <div className="text-center w-full max-w-xs">
              ...
              <h3 className="text-lg font-medium text-zinc-900 dark:text-zinc-100 mb-2">
                {t('toolViews.webCrawl.scrapingWebpage')}
              </h3>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-sm font-medium text-zinc-800 dark:text-zinc-200">
                    ...
                    {t('toolViews.webCrawl.extractedContent')}
                  </div>
                  {contentStats && (
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="text-xs">
                        {contentStats.wordCount} {t('toolViews.webCrawl.words')}
                      </Badge>
                      <Badge variant="outline" className="text-xs">
                        {contentStats.charCount} {t('toolViews.webCrawl.chars')}
                      </Badge>
                    </div>
                  )}
                </div>
                ...
                {webpageContent?.text ? (
                  <div className="group relative bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden hover:border-zinc-300 dark:hover:border-zinc-700 transition-all duration-200 hover:shadow-sm">
                    {/* Content Header */}
                    <div className="flex items-center justify-between p-3 bg-[#F2F5FF] dark:bg-zinc-800/50 border-b border-zinc-200 dark:border-zinc-700">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100">
                            {t('toolViews.webCrawl.pageContent')}
                          </p>
                          {contentStats && (
                            <p className="text-xs text-zinc-500 dark:text-zinc-400">
                              {contentStats.lineCount} {t('toolViews.webCrawl.lines')}
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1">
                        <TooltipProvider>
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <p>{copiedContent ? t('toolViews.webCrawl.copied') : t('toolViews.webCrawl.copyContent')}</p>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </div>
                  ...
                  <div className="text-center py-12 px-6 bg-gradient-to-b from-white to-zinc-50 dark:from-zinc-950 dark:to-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800">
                    ...
                    <h3 className="text-lg font-medium mb-2 text-zinc-900 dark:text-zinc-100">
                      {t('toolViews.webCrawl.noContentExtracted')}
                    </h3>
                    <p className="text-sm text-zinc-500 dark:text-zinc-400 max-w-sm mx-auto">
                      {t('toolViews.webCrawl.webpageRestrictedEmptyOrRequireJavaScript')}
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="flex flex-col items-center justify-center h-full py-12 px-6 bg-gradient-to-b from-white to-zinc-50 dark:from-zinc-950 dark:to-zinc-900">
            ...
            <h3 className="text-xl font-semibold mb-2 text-zinc-900 dark:text-zinc-100">
              {t('toolViews.webCrawl.noUrlDetected')}
            </h3>
            <p className="text-zinc-500 dark:text-zinc-400 text-center max-w-sm">
              {t('toolViews.webCrawl.unableToExtractValidUrl')}
            </p>
          </div>
        )}
        {/* Footer */}
