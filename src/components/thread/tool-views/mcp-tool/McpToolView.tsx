        ...
        <div className="flex flex-row items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                {t('toolViews.mcp.externalTool')} • {serverName}
              </p>
            </div>
          </div>
          ...
          {!isStreaming && (
            ...
            >
              ...
              {isSuccess && result && !result.isError ? t('toolViews.mcp.toolExecuted') : t('toolViews.mcp.toolFailed')}
            </Badge>
          )}
        </div>
      </CardHeader>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="bg-[#F2F5FF]/70 dark:bg-zinc-900/30 p-4 rounded-lg border border-zinc-100 dark:border-zinc-800">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div>
                    <span className="text-zinc-500 dark:text-zinc-400">{t('toolViews.mcp.serverName')}:</span>
                    <span className="ml-2 font-medium text-zinc-700 dark:text-zinc-300">
                      {serverName}
                    </span>
                  </div>
                  <div>
                    <span className="text-zinc-500 dark:text-zinc-400">{t('toolViews.mcp.toolName')}:</span>
                    <span className="ml-2 font-medium text-zinc-700 dark:text-zinc-300">
                      {toolName}
                    </span>
                  </div>
                  <div>
                    <span className="text-zinc-500 dark:text-zinc-400">{t('toolViews.common.input')}:</span>
                    <span className="ml-2 font-medium text-zinc-700 dark:text-zinc-300">
                      {argumentsCount} parameter{argumentsCount !== 1 ? 's' : ''}
                    </span>
                  </div>
                  <div>
                    <span className="text-zinc-500 dark:text-zinc-400">{t('toolViews.common.status')}:</span>
                    <span className={cn(
                      "ml-2 font-medium",
                      isSuccess && result && !result.isError
                      ...
                      {isSuccess && result && !result.isError ? t('toolViews.common.success') : t('toolViews.common.error')}
                    </span>
                  </div>
                </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-zinc-200 dark:border-zinc-700">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-red-600 dark:text-red-400">
                        {t('toolViews.mcp.errorDetails')}: {result.error_type}
                      </span>
                    </div>
                  </div>
                )}
