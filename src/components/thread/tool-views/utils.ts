...
// Get localized tool title using translation function
export function getLocalizedToolTitle(toolName: string, t: (key: string) => string): string {
  // Normalize tool name
  const normalizedName = toolName.toLowerCase();

  // Map of tool names to their translation keys
  const toolTitleKeys: Record<string, string> = {
    ...
    'browser-navigate': 'navigatingToPage',
    'browser-click': 'clickingElement',
    'browser-extract': 'extractingContent',
    'browser-fill': 'enteringText',
    ...
    'generic-tool': 'externalTool',
    'default': 'externalTool',
  };

  // Return the localized title if we have a translation key
  if (toolTitleKeys[normalizedName]) {
    return t(`toolViews.toolNames.${toolTitleKeys[normalizedName]}`);
  }

  // For browser tools not explicitly mapped
  if (normalizedName.startsWith('browser-')) {
    const operation = normalizedName.replace('browser-', '');
    const operationKey = toolTitleKeys[`browser-${operation}`];
    if (operationKey) {
      return t(`toolViews.toolNames.${operationKey}`);
    }
    // Fallback to formatted name
    const formattedOperation = operation.replace(/-/g, ' ');
    return 'Browser ' + formattedOperation.charAt(0).toUpperCase() + formattedOperation.slice(1);
  }

  // Fallback to original function for unmapped tools
