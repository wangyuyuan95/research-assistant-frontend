...
export const STORAGE_KEY_MODEL = 'research-assistant-preferred-model-v2';
export const STORAGE_KEY_CUSTOM_MODELS = 'customModels';
export const DEFAULT_FREE_MODEL_ID = 'deepseek';
export const DEFAULT_PREMIUM_MODEL_ID = 'auto';
// export const DEFAULT_PREMIUM_MODEL_ID = 'google/gemini-2.5-pro-preview';
...
export const MODELS = {
  // Auto selection - highest priority
  'auto': {
    tier: 'free', // Available to all users
    priority: 200, // Highest priority
    recommended: true,
    lowQuality: false,
    description: 'models.autoDescription' // Use translation key
  },
...
export const useModelSelection = () => {
  ...
  const MODEL_OPTIONS = useMemo(() => {
    ...
    models.push({
      id: 'auto', 
      label: 'auto', // This will be translated in the component
      requiresSubscription: false,
      description: MODELS['auto']?.description || 'Auto-select best model',
      priority: MODELS['auto']?.priority || 200,
      recommended: MODELS['auto']?.recommended || true
    });
    
    // Default models if API data not available or failed to load
    if (!modelsData?.models || isLoadingModels || !Array.isArray(modelsData.models) || modelsData.models.length === 0) {
      models.push(
        { 
        ...
        { 
          ...
          requiresSubscription: true, 
          description: MODELS['claude-sonnet-4']?.description || MODEL_TIERS.premium.baseDescription,
          priority: MODELS['claude-sonnet-4']?.priority || 100
        }
      );
    } else {
      // Process API-provided models and add them
      const apiModels = modelsData.models.map(model => {
    ...
    try {
      ...
      if (isLocalMode()) {
        if (savedModel && MODEL_OPTIONS.find(option => option.id === savedModel)) {
          ...
          setSelectedModel('auto'); // Default to auto instead of DEFAULT_PREMIUM_MODEL_ID
  ...
  const getActualModelId = (modelId: string): string => {
    // If auto is selected, return empty string to let backend choose
    if (modelId === 'auto') {
      return '';
    }
