...
export const ModelSelector: React.FC<ModelSelectorProps> = ({
  ...
  // Convert map back to array and enhance with translations
  const enhancedModelOptions = Array.from(modelMap.values()).map(model => {
    // Special handling for auto option translation
    if (model.id === 'auto') {
      return {
        ...model,
        label: t('models.auto'),
        description: t('models.autoDescription')
      };
    }
    return model;
  });
  ...
  const selectedLabel =
    enhancedModelOptions.find((o) => o.id === selectedModel)?.label || t('models.selectModel');
        ...
        <Tooltip>
          ...
          {!accessible ? (
            <TooltipContent side="left" className="text-xs max-w-xs">
              <p>{t('models.requiresSubscription')}</p>
            </TooltipContent>
          ) : isLowQuality ? (
            <TooltipContent side="left" className="text-xs max-w-xs">
              <p>{t('models.basicModelLimited')}</p>
            </TooltipContent>
          ) : isRecommended ? (
            <TooltipContent side="left" className="text-xs max-w-xs">
              <p>{t('models.recommendedForOptimal')}</p>
            </TooltipContent>
          ) : isCustom ? (
            <TooltipContent side="left" className="text-xs max-w-xs">
              <p>{t('models.customModel')}</p>
            </TooltipContent>
          ) : null}
        </Tooltip>
      </TooltipProvider>
    ...
    <div className="relative">
            ...
            {shouldDisplayAll ? (
                        ...
                        <div className="rounded-xl bg-gradient-to-br from-blue-50/80 to-blue-200/70 dark:from-blue-950/40 dark:to-blue-900/30 shadow-sm border border-blue-200/50 dark:border-blue-800/50 p-3">
                          <div className="flex flex-col space-y-2">
                            <div className="flex items-center">
                              ...
                              <div>
                                <p className="text-sm font-medium">{t('models.unlockAllModels')}</p>
                              </div>
                            </div>
                            <Button
                              size="sm"
              ...
              <div className='max-h-[320px] overflow-y-auto w-full'>
                <div className="px-3 py-3 flex justify-between items-center">
                  <span className="text-xs font-medium text-muted-foreground">{t('models.allModels')}</span>
                  {/* {isLocalMode() && (
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
      ...
      {paywallOpen && (
        <PaywallDialog
          ...
          title={t('premiumModel')}
          description={
            lockedModel
              ? `${enhancedModelOptions.find(m => m.id === lockedModel)?.label || 'This model'} requires a premium subscription to access its enhanced features`
              : t('subscribeToAccessPremium')
          }
          ctaText={t('subscribeNow')}
