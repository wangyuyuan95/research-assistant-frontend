...
// Helper function to clean message content for copying
...
// Helper function to copy text to clipboard with timeout
...
export const ThreadContent: React.FC<ThreadContentProps> = ({
    ...
    isPreviewMode = false,
    agentName,
    agentAvatar = <KortixLogo size={18} />,
    emptyStateComponent,
}) => {
    ...
    const containerClassName = isPreviewMode 
        ? "flex-1 overflow-y-auto scrollbar-thin scrollbar-track-secondary/0 scrollbar-thumb-primary/10 scrollbar-thumb-rounded-full hover:scrollbar-thumb-primary/10 px-6 pt-[26px] pb-[248px]"
        : "flex-1 overflow-y-auto scrollbar-thin scrollbar-track-secondary/0 scrollbar-thumb-primary/10 scrollbar-thumb-rounded-full hover:scrollbar-thumb-primary/10 px-6 pt-[26px] pb-[248px] bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60";

    // In playback mode, we use visibleMessages instead of messages
    const rawDisplayMessages = readOnly && visibleMessages ? visibleMessages : messages;
    
    // Filter out hidden messages (messages with <hidden> tags)
    const displayMessages = rawDisplayMessages.filter(message => !shouldHideMessage(message));
    ...
    const { copiedMap: userCopiedMap, copy: copyUserMessage } = useCopyToClipboard();
    const { copiedMap: assistantCopiedMap, copy: copyAssistantMessage } = useCopyToClipboard();
                                    ...
                                    if (group.type === 'user') {
                                        ...
                                        return (
                                            <div key={group.key} className="flex flex-col items-end gap-2 mb-3">
                                                ...
                                                {/* User message timestamp and copy button */}
                                                {message.created_at && (
                                                    <div className="flex items-center gap-2 text-xs text-muted-foreground px-2">
                                                        ...
                                                        <Tooltip>
                                                            ...
                                                            <TooltipContent>
                                                                {userCopiedMap[message.message_id] ? t('common.copied') : t('chat.copyQuestion')}
                                                            </TooltipContent>
                                                        </Tooltip>
                                                    </div>
                                                )}
                                                                                                ...
                                                                                                >
                                                                                                    ...
                                                                                                    {paramDisplay && (
                                                                                                        paramDisplay.length > 30 ? (
                                                                                                            <Tooltip>
                                                                                                                <TooltipTrigger asChild>
                                                                                                                    <span className="ml-1 text-primary/70 truncate max-w-[200px] cursor-help">{paramDisplay}</span>
                                                                                                                </TooltipTrigger>
                                                                                                                <TooltipContent>
                                                                                                                    {paramDisplay}
                                                                                                                </TooltipContent>
                                                                                                            ...
                                                                                                            <span className="ml-1 text-primary/70 truncate max-w-[200px]" title={paramDisplay}>{paramDisplay}</span>
                                                                                                        )
                                                                                                    )}
                                                                                                </button>
                                                                                            );
                                                                                                ...
                                                                                                >
                                                                                                    ...
                                                                                                    <span className="font-mono text-xs text-primary">
                                                                                                        {detectedTag === 'function_calls' ? (extractToolNameFromStream(streamingText) || 'Using Tool...') : detectedTag}
                                                                                                            ...
                                                                                                            {/* Assistant message timestamp - use the first assistant message in the group */}
                                                                                                            {(() => {
                                                            const firstAssistantMessage = group.messages.find(msg => msg.type === 'assistant');
                                                            const messageContent = firstAssistantMessage ? safeJsonParse<ParsedContent>(firstAssistantMessage.content, {}).content || '' : '';
                                                            // 检查是否是最终回答（包含完成任务、询问标签或者完全不包含<function_calls></function_calls>标签的回答）
                                                            const isFinalAnswer = messageContent && (
                                                                // 检查旧格式
                                                                messageContent.includes('</complete>') || 
                                                                messageContent.includes('</ask>') ||
                                                                !messageContent.includes('<function_calls>') ||
                                                                // 检查新格式
                                                                (messageContent.includes('<function_calls>') && 
                                                                    (messageContent.includes('name="complete"') || 
                                                                     messageContent.includes('name="ask"')))
                                                            );
                                                            return firstAssistantMessage?.created_at ? (
                                                                <div className="flex items-center gap-2 ml-2 text-xs text-muted-foreground">
                                                                    <span>{formatTimestamp(firstAssistantMessage.created_at)}</span>
                                                                    {isFinalAnswer && messageContent && (
                                                                        <>
                                                                            <span className="text-muted-foreground/30">|</span>
                                                                            <Tooltip>
                                                                                <TooltipTrigger asChild>
                                                                                ...
                                                                                <TooltipContent>
                                                                                    {assistantCopiedMap[firstAssistantMessage.message_id] ? t('common.copied') : t('chat.copyAnswer')}
                                                                                </TooltipContent>
                                                                            </Tooltip>
                                                                        </>
                                                                    )}
