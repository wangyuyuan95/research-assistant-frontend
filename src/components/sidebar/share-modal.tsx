...
export function ShareModal({ isOpen, onClose, threadId, projectId }: ShareModalProps) {
  ...
  const createShareLink = async () => {
    ...
    try {
      ...
      toast.success(t('share.linkCreatedSuccessfully'))
    } catch (error) {
      console.error("Error creating share link:", error)
      toast.error(t('share.failedToCreateLink'))
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const removeShareLink = async () => {
    ...
    try {
      ...
      toast.error(t('share.failedToRemoveLink'))
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const socialOptions: SocialShareOption[] = [
    {
      ...
      onClick: () => {
        if (shareLink) {
          window.open(
            `https://www.linkedin.com/shareArticle?url=${encodeURIComponent(shareLink)}&text=${encodeURIComponent(t('share.sharedConversation'))}`,
            "_blank",
          )
        }
      },
    },
    {
      ...
      onClick: () => {
        if (shareLink) {
          window.open(
            `https://twitter.com/intent/tweet?url=${encodeURIComponent(shareLink)}&text=${encodeURIComponent(t('share.sharedConversation'))}`,
            "_blank",
          )
        }
      },
