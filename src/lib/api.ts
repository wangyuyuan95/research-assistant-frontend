...
import { getDatasetIds, getKbApiKey, getKbUrl } from '@/lib/kb-integration';
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.log(`[API] Raw error response for ${response.status}:`, errorText);
      ...
      if (response.status === 402) {
        if (errorData?.detail) {
          ...
          console.error('[API] Could not parse 402 error response body');
      ...
      // Handle 422 Usage Limit error
      if (response.status === 422) {
        if (errorData?.detail?.message) {
          console.log('[API] Using error message from detail:', errorData.detail.message);
          throw new Error(errorData.detail.message);
        } else {
          console.log('[API] No detail message found, using default');
...
export const initiateAgent = async (
  ...
  try {
    ...
    const addKnowledgeBaseParams = () => {
      ...
      const datasetIds = getDatasetIds();
      if (datasetIds.length > 0) {
        formData.append('dataset_ids', JSON.stringify(datasetIds));
      }
      
      // 获取kb_api_key
      const kbApiKey = getKbApiKey();
      if (kbApiKey !== null) {
        formData.append('kb_api_key', kbApiKey);
      }
      
      // 获取kb_url
      const kbUrl = getKbUrl();
      if (kbUrl !== null) {
        formData.append('kb_url', kbUrl);
      }
    };
